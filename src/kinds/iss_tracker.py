...
def build_html(niche: str, raw_data: dict, sources: list) -> str:
    ...
    html_out = (
        ...
        '<tbody>'
        f'<tr><th>📍 現在位置（緯度）</th><td>{lat_dir} {lat_abs:.4f}°</td></tr>'
        f'<tr><th>📍 現在位置（経度）</th><td>{lon_dir} {lon_abs:.4f}°</td></tr>'
        f'<tr><th>🌐 高度</th><td>約 {altitude:.1f} km</td></tr>'
        f'<tr><th>⚡ 速度</th><td>{velocity:.1f} km/h（約 {velocity/3600:.2f} km/s）</td></tr>'
        f'<tr><th>☀️ 照明状態</th><td>{visibility_label}</td></tr>'
        f'<tr><th>📡 観測フットプリント直径</th><td>{html.escape(footprint_str)}</td></tr>'
        f'<tr><th>🕐 取得時刻（UTC）</th><td>{dt_utc}</td></tr>'
        '</tbody>'
        '</table>'
        '<p>'
        f'🗺️ 地図で確認: '
