...
def build_html(niche: str, raw_data: dict, sources: list) -> str:
    ...
    for book in books:
        ...
        rows.append(
            ...
            f'<div style="font-size:0.85em;color:#c00;font-weight:bold;">{price if price else ""}</div>'
            f'{desc_html}'
            f'</td>'
            f'<td style="text-align:center;vertical-align:top;padding:6px;white-space:nowrap;">'
            f'<div style="font-size:0.8em;color:#888;">{isbn}</div>'
    ...
    html_out = (
        ...
        f'取得件数: <strong>{total}件</strong>　最終取得: {fetched_at}'
        f'</div>'
