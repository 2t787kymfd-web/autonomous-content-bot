"""kind_generator.py が自動生成したプラグイン: book_isbn"""

import requests
import json
import html
import re
from datetime import datetime

KIND_NAME = "book_isbn"
KEYWORDS = ["書籍", "ISBN", "本", "図書", "openBD", "書誌情報", "出版"]
CATEGORY = "地理・開発者向け"

# 検索に使うサンプルISBNリスト(ジャンル別・人気書籍)
SAMPLE_ISBNS = [
    "9784065149232",  # 嫌われる勇気
    "9784101092058",  # 人間失格
    "9784167110123",  # 吾輩は猫である
    "9784062748895",  # 坊っちゃん
    "9784003101773",  # こころ
    "9784569825816",  # 影響力の武器
    "9784478025819",  # ゼロ秒思考
    "9784763136497",  # 夢をかなえるゾウ
    "9784344028685",  # コンビニ人間
    "9784101304519",  # 雪国
    "9784167110086",  # 羅生門
    "9784003110287",  # 銀河鉄道の夜
    "9784408536200",  # ハリー・ポッターと賢者の石
    "9784101181776",  # 伊豆の踊子
    "9784062753791",  # 容疑者Xの献身
]

# 検索用テキストの正規化で除去する区切り文字。openBDの著者名は
# 「姓,名,生年-没年」のようなカンマ区切りで返ってくるため、区切りを
# 跨いだキーワード(例: 「東野圭吾」)がそのままでは部分一致しない。
# 索引側・クエリ側の両方からこれらの区切り文字を除去して比較することで、
# 表記の区切り方に関わらず部分一致検索できるようにする
# (下のbuild_html()内の埋め込みJSにも同じ文字クラスを複製している)。
_SEARCH_SEPARATORS_RE = re.compile(r"[,、\s:：\-–—()（）./]")


def _normalize_for_search(text: str) -> str:
    return _SEARCH_SEPARATORS_RE.sub("", text)


def fetch(niche: str) -> tuple:
    """researcher.pyの契約: (summary: str, sources: list, raw_data: dict) を返す。"""
    # 複数のISBNを試して実データが取れるものを集める
    found_books = []

    # ISBNをバッチ取得(最大10件まとめて取得)
    isbn_batch = ",".join(SAMPLE_ISBNS[:10])
    url = f"https://api.openbd.jp/v1/get?isbn={isbn_batch}&pretty=0"
    resp = requests.get(url, timeout=15)
    resp.raise_for_status()
    data = resp.json()

    for item in data:
        if item is None:
            continue
        summary_obj = item.get("summary", {})
        onix = item.get("onix", {})
        if not summary_obj:
            continue

        title = summary_obj.get("title", "")
        if not title:
            continue

        isbn = summary_obj.get("isbn", "")
        author = summary_obj.get("author", "")
        publisher = summary_obj.get("publisher", "")
        pubdate = summary_obj.get("pubdate", "")
        cover = summary_obj.get("cover", "")

        # 概要テキスト取得
        description = ""
        try:
            col_detail = onix.get("CollateralDetail", {})
            text_contents = col_detail.get("TextContent", [])
            for tc in text_contents:
                if tc.get("TextType") in ("02", "03", "04"):
                    description = tc.get("Text", "")
                    if description:
                        break
            if not description and text_contents:
                description = text_contents[0].get("Text", "")
        except Exception:
            description = ""

        # 価格取得
        price_str = ""
        try:
            product_supply = onix.get("ProductSupply", {})
            supply_detail = product_supply.get("SupplyDetail", {})
            prices = supply_detail.get("Price", [])
            if isinstance(prices, list) and prices:
                amount = prices[0].get("PriceAmount", "")
                if amount:
                    price_str = f"{amount}円"
            elif isinstance(prices, dict):
                amount = prices.get("PriceAmount", "")
                if amount:
                    price_str = f"{amount}円"
        except Exception:
            price_str = ""

        # 出版日フォーマット
        pubdate_fmt = ""
        if pubdate and len(pubdate) >= 6:
            try:
                y = pubdate[:4]
                m = pubdate[4:6]
                d = pubdate[6:8] if len(pubdate) >= 8 else ""
                pubdate_fmt = f"{y}年{m}月" + (f"{d}日" if d else "")
            except Exception:
                pubdate_fmt = pubdate

        found_books.append({
            "isbn": isbn,
            "title": title,
            "author": author,
            "publisher": publisher,
            "pubdate": pubdate_fmt,
            "cover": cover,
            "description": description[:200] if description else "",
            "price": price_str,
        })

    # 残りのISBNも試す
    if len(SAMPLE_ISBNS) > 10:
        isbn_batch2 = ",".join(SAMPLE_ISBNS[10:])
        try:
            url2 = f"https://api.openbd.jp/v1/get?isbn={isbn_batch2}&pretty=0"
            resp2 = requests.get(url2, timeout=15)
            resp2.raise_for_status()
            data2 = resp2.json()
            for item in data2:
                if item is None:
                    continue
                summary_obj = item.get("summary", {})
                onix = item.get("onix", {})
                if not summary_obj:
                    continue
                title = summary_obj.get("title", "")
                if not title:
                    continue
                isbn = summary_obj.get("isbn", "")
                author = summary_obj.get("author", "")
                publisher = summary_obj.get("publisher", "")
                pubdate = summary_obj.get("pubdate", "")
                cover = summary_obj.get("cover", "")
                description = ""
                try:
                    col_detail = onix.get("CollateralDetail", {})
                    text_contents = col_detail.get("TextContent", [])
                    for tc in text_contents:
                        if tc.get("TextType") in ("02", "03", "04"):
                            description = tc.get("Text", "")
                            if description:
                                break
                    if not description and text_contents:
                        description = text_contents[0].get("Text", "")
                except Exception:
                    description = ""
                price_str = ""
                try:
                    product_supply = onix.get("ProductSupply", {})
                    supply_detail = product_supply.get("SupplyDetail", {})
                    prices = supply_detail.get("Price", [])
                    if isinstance(prices, list) and prices:
                        amount = prices[0].get("PriceAmount", "")
                        if amount:
                            price_str = f"{amount}円"
                    elif isinstance(prices, dict):
                        amount = prices.get("PriceAmount", "")
                        if amount:
                            price_str = f"{amount}円"
                except Exception:
                    price_str = ""
                pubdate_fmt = ""
                if pubdate and len(pubdate) >= 6:
                    try:
                        y = pubdate[:4]
                        m = pubdate[4:6]
                        d = pubdate[6:8] if len(pubdate) >= 8 else ""
                        pubdate_fmt = f"{y}年{m}月" + (f"{d}日" if d else "")
                    except Exception:
                        pubdate_fmt = pubdate
                found_books.append({
                    "isbn": isbn,
                    "title": title,
                    "author": author,
                    "publisher": publisher,
                    "pubdate": pubdate_fmt,
                    "cover": cover,
                    "description": description[:200] if description else "",
                    "price": price_str,
                })
        except Exception:
            pass

    if not found_books:
        raise RuntimeError("openBD APIから書籍データを1件も取得できませんでした")

    titles_sample = "、".join([b["title"] for b in found_books[:3]])
    summary = (
        f"openBD APIから{len(found_books)}件の書籍情報を取得しました。"
        f"収録例: {titles_sample} など。"
        f"タイトル・著者・出版社・出版日・価格・あらすじを掲載。"
    )
    sources = ["https://api.openbd.jp/ (openBD - 書誌情報・書影提供サービス)"]
    raw = {
        "books": found_books,
        "total": len(found_books),
        "fetched_at": datetime.utcnow().strftime("%Y-%m-%d %H:%M UTC"),
    }
    return summary, sources, raw


def build_html(niche: str, raw_data: dict, sources: list) -> str:
    """tool_builder.pyの契約: ページ本文のHTML断片を返す。"""
    books = raw_data.get("books", [])
    total = raw_data.get("total", 0)
    fetched_at = html.escape(str(raw_data.get("fetched_at", "")))

    source_text = html.escape(sources[0]) if sources else "openBD"

    rows = []
    for book in books:
        isbn = html.escape(str(book.get("isbn", "")))
        title = html.escape(str(book.get("title", "")))
        author = html.escape(str(book.get("author", "")))
        publisher = html.escape(str(book.get("publisher", "")))
        pubdate = html.escape(str(book.get("pubdate", "")))
        price = html.escape(str(book.get("price", "")))
        description = html.escape(str(book.get("description", "")))
        cover_raw = book.get("cover", "")
        # cover URLの安全確認
        cover_img = ""
        if cover_raw and cover_raw.startswith("https://"):
            cover_safe = html.escape(cover_raw)
            cover_img = f'<img src="{cover_safe}" alt="{title}" style="max-width:80px;max-height:110px;object-fit:contain;">'
        else:
            cover_img = '<span style="color:#aaa;font-size:0.8em;">書影なし</span>'

        # Amazonリンク(ISBNから)
        if isbn:
            amazon_url = f"https://www.amazon.co.jp/dp/{isbn}"
            amazon_link = f'<a href="{html.escape(amazon_url)}" target="_blank" rel="noopener">Amazon</a>'
        else:
            amazon_link = "-"

        # 説明文(長い場合は省略)
        desc_html = ""
        if description:
            desc_html = f'<div style="font-size:0.82em;color:#555;margin-top:4px;">{description}</div>'

        # タイトル・著者・出版社・価格・あらすじを横断検索できるよう、
        # 行ごとに検索用テキスト(小文字結合・区切り文字除去)をdata属性へ
        # 埋め込む(下のJSが同じ正規化を施した入力値で部分一致フィルタする)。
        search_blob = html.escape(
            _normalize_for_search(
                " ".join([title, author, publisher, price, description]).lower()
            )
        )

        rows.append(
            f'<tr data-search="{search_blob}">'
            f'<td style="text-align:center;vertical-align:top;padding:6px;">{cover_img}</td>'
            f'<td style="vertical-align:top;padding:6px;">'
            f'<strong>{title}</strong>'
            f'<div style="font-size:0.88em;color:#333;margin-top:2px;">著者: {author if author else "不明"}</div>'
            f'<div style="font-size:0.85em;color:#555;">出版社: {publisher if publisher else "不明"} ／ 発行: {pubdate if pubdate else "不明"}</div>'
            f'<div style="font-size:0.85em;color:#c00;font-weight:bold;" class="tel-value">{price if price else ""}</div>'
            f'{desc_html}'
            f'</td>'
            f'<td style="text-align:center;vertical-align:top;padding:6px;white-space:nowrap;">'
            f'<div style="font-size:0.8em;color:#888;" class="tel-value">{isbn}</div>'
            f'<div style="margin-top:4px;">{amazon_link}</div>'
            f'</td>'
            f'</tr>'
        )

    rows_html = "\n".join(rows)

    table_head_html = (
        '<thead>'
        '<tr style="background:#f0f0f0;">'
        '<th style="padding:8px;text-align:center;width:90px;">書影</th>'
        '<th style="padding:8px;text-align:left;">書籍情報</th>'
        '<th style="padding:8px;text-align:center;width:120px;">ISBN / リンク</th>'
        '</tr>'
        '</thead>'
    )

    # --- 自動取得された書籍一覧(常に全件表示。検索結果とは別ブロック) ---
    auto_list_html = (
        f'<div class="tool-section">'
        f'<h2>自動取得された書籍一覧</h2>'
        f'<div style="margin-bottom:12px;font-size:0.9em;color:#555;">'
        f'取得件数: <strong class="tel-value">{total}件</strong>　最終取得: {fetched_at}'
        f'</div>'
        f'<div style="overflow-x:auto;">'
        f'<table id="book-table" style="width:100%;border-collapse:collapse;">'
        f'{table_head_html}'
        f'<tbody>{rows_html}</tbody>'
        f'</table>'
        f'</div>'
        f'</div>'
    )

    # --- タイトル・著者・出版社・価格・あらすじの横断検索。
    # 上の自動取得一覧はそのまま(絞り込み等で書き換えない)にし、
    # 一致した書籍だけをこの専用ブロックに複製して表示する。 ---
    filter_html = (
        '<div class="tool-section">'
        '<h2>キーワードで検索</h2>'
        '<p>タイトル・著者・出版社・価格・あらすじのいずれかに一致する書籍を、'
        '上の自動取得一覧とは別にここへ表示します。</p>'
        '<input id="book-filter" type="text"'
        ' placeholder="例: 夏目漱石 / 講談社 / 1500円"'
        ' style="padding:8px 12px;font-size:1rem;border:1px solid #ccc;'
        'border-radius:4px;width:100%;max-width:360px;" />'
        '<p id="book-filter-count" style="font-size:0.85em;color:#888;margin-top:6px;"></p>'
        '<p id="book-search-empty" style="font-size:0.9em;color:#888;margin-top:8px;">'
        'キーワードを入力すると、一致する書籍がここに表示されます。'
        '</p>'
        '<div id="book-search-results-wrap" style="overflow-x:auto;display:none;">'
        f'<table id="book-search-results" style="width:100%;border-collapse:collapse;">'
        f'{table_head_html}'
        '<tbody id="book-search-tbody"></tbody>'
        '</table>'
        '</div>'
        '</div>'
    )

    # --- ISBN直接検索フォーム(一覧に無い任意のISBNもopenBDへその場で問い合わせる) ---
    isbn_search_html = (
        '<div class="tool-section">'
        '<h2>ISBNで検索</h2>'
        '<p>13桁のISBNコードを入力すると、上の一覧に無い書籍もその場で検索できます'
        '（ハイフンあり・なし両方対応）。</p>'
        '<div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;">'
        '<input id="isbn-input" type="text" placeholder="例: 9784065149232 または 978-4-06-514923-2"'
        ' maxlength="20" style="padding:8px 12px;font-size:1rem;border:1px solid #ccc;'
        'border-radius:4px;width:240px;" />'
        '<button onclick="searchIsbnBook()" style="padding:8px 18px;font-size:1rem;'
        'background:#0066cc;color:#fff;border:none;border-radius:4px;cursor:pointer;">検索</button>'
        '</div>'
        '<div id="isbn-search-result" style="margin-top:16px;"></div>'
        '</div>'
        '<script>'
        'function normalizeForSearch(s) {'
        '  return s.replace(/[,、\\s:：\\-\\u2013\\u2014()（）./]/g, "");'
        '}'
        'var BOOK_SEARCH_HINT = "キーワードを入力すると、一致する書籍がここに表示されます。";'
        'var BOOK_SEARCH_NO_MATCH = "一致する書籍が見つかりませんでした。";'
        'document.getElementById("book-filter").addEventListener("input", function () {'
        '  var q = normalizeForSearch(this.value.trim().toLowerCase());'
        '  var sourceRows = document.querySelectorAll("#book-table tbody tr");'
        '  var resultsBody = document.getElementById("book-search-tbody");'
        '  var emptyMsg = document.getElementById("book-search-empty");'
        '  var resultsWrap = document.getElementById("book-search-results-wrap");'
        '  var countEl = document.getElementById("book-filter-count");'
        '  resultsBody.innerHTML = "";'
        '  if (q === "") {'
        '    emptyMsg.textContent = BOOK_SEARCH_HINT;'
        '    emptyMsg.style.display = "";'
        '    resultsWrap.style.display = "none";'
        '    countEl.textContent = "";'
        '    return;'
        '  }'
        '  var shown = 0;'
        '  sourceRows.forEach(function (tr) {'
        '    var blob = tr.getAttribute("data-search") || "";'
        '    if (blob.indexOf(q) !== -1) {'
        '      resultsBody.appendChild(tr.cloneNode(true));'
        '      shown++;'
        '    }'
        '  });'
        '  if (shown === 0) {'
        '    emptyMsg.textContent = BOOK_SEARCH_NO_MATCH;'
        '    emptyMsg.style.display = "";'
        '    resultsWrap.style.display = "none";'
        '  } else {'
        '    emptyMsg.style.display = "none";'
        '    resultsWrap.style.display = "";'
        '  }'
        '  countEl.textContent = shown + "件ヒットしました。";'
        '});'
        'function escHtmlBook(s) {'
        '  return String(s).replace(/&/g,"&amp;").replace(/</g,"&lt;")'
        '    .replace(/>/g,"&gt;").replace(/"/g,"&quot;");'
        '}'
        'function searchIsbnBook() {'
        '  var raw = document.getElementById("isbn-input").value;'
        '  var code = raw.replace(/[^0-9Xx]/g, "");'
        '  var el = document.getElementById("isbn-search-result");'
        '  if (!code) {'
        '    el.innerHTML = "<p style=color:red>ISBNを入力してください。</p>";'
        '    return;'
        '  }'
        '  el.innerHTML = "<p>検索中...</p>";'
        '  fetch("https://api.openbd.jp/v1/get?isbn=" + encodeURIComponent(code))'
        '    .then(function (r) { return r.json(); })'
        '    .then(function (data) {'
        '      var item = data && data[0];'
        '      if (!item || !item.summary || !item.summary.title) {'
        '        el.innerHTML = "<p style=color:red>該当する書籍が見つかりませんでした。</p>";'
        '        return;'
        '      }'
        '      var s = item.summary;'
        '      var onix = item.onix || {};'
        '      var title = s.title || "";'
        '      var author = s.author || "";'
        '      var publisher = s.publisher || "";'
        '      var cover = s.cover || "";'
        '      var priceStr = "";'
        '      try {'
        '        var prices = onix.ProductSupply.SupplyDetail.Price;'
        '        var p = Array.isArray(prices) ? prices[0] : prices;'
        '        if (p && p.PriceAmount) priceStr = p.PriceAmount + "円";'
        '      } catch (e) {}'
        '      var desc = "";'
        '      try {'
        '        var tcs = onix.CollateralDetail.TextContent;'
        '        for (var i = 0; i < tcs.length; i++) {'
        '          if (["02","03","04"].indexOf(tcs[i].TextType) !== -1 && tcs[i].Text) {'
        '            desc = tcs[i].Text; break;'
        '          }'
        '        }'
        '        if (!desc && tcs.length) desc = tcs[0].Text || "";'
        '      } catch (e) {}'
        '      var coverHtml = (cover && cover.indexOf("https://") === 0)'
        '        ? "<img src=\'" + escHtmlBook(cover) + "\' alt=\'\' style=\'max-width:80px;'
        'max-height:110px;object-fit:contain;\'>"'
        '        : "<span style=color:#aaa;font-size:0.8em;>書影なし</span>";'
        '      var amazonHtml = "<a href=\'https://www.amazon.co.jp/dp/" + escHtmlBook(code) +'
        '        "\' target=_blank rel=noopener>Amazon</a>";'
        '      el.innerHTML ='
        '        "<div style=\'display:flex;gap:12px;border:1px solid #ccc;border-radius:6px;padding:10px;\'>" +'
        '        "<div>" + coverHtml + "</div>" +'
        '        "<div>" +'
        '        "<strong>" + escHtmlBook(title) + "</strong>" +'
        '        "<div style=\'font-size:0.88em;color:#333;margin-top:2px;\'>著者: " +'
        '        (author ? escHtmlBook(author) : "不明") + "</div>" +'
        '        "<div style=\'font-size:0.85em;color:#555;\'>出版社: " +'
        '        (publisher ? escHtmlBook(publisher) : "不明") + "</div>" +'
        '        (priceStr ? "<div style=\'font-size:0.85em;color:#c00;font-weight:bold;\'>" +'
        '          escHtmlBook(priceStr) + "</div>" : "") +'
        '        (desc ? "<div style=\'font-size:0.82em;color:#555;margin-top:4px;\'>" +'
        '          escHtmlBook(desc.slice(0, 200)) + "</div>" : "") +'
        '        "<div style=\'margin-top:4px;\'>" + amazonHtml + "</div>" +'
        '        "</div></div>";'
        '    })'
        '    .catch(function () {'
        '      el.innerHTML = "<p style=color:red>通信エラーが発生しました。</p>";'
        '    });'
        '}'
        'document.getElementById("isbn-input").addEventListener("keydown", function (e) {'
        '  if (e.key === "Enter") searchIsbnBook();'
        '});'
        '</script>'
    )

    html_out = (
        f'<h1>📚 {html.escape(niche)}</h1>'
        f'<p>'
        f'openBD APIを利用して日本の書籍情報をISBNコードから検索・表示します。'
        f'タイトル・著者・出版社・価格・あらすじを一覧で確認できます。'
        f'</p>'
        + auto_list_html
        + filter_html
        + isbn_search_html
        + f'<div class="source">'
        f'データ出典: <a href="https://api.openbd.jp/" target="_blank" rel="noopener">openBD</a>'
        f'（出版社・取次が提供する書誌情報・書影を無償提供）'
        f'</div>'
    )
    return html_out

